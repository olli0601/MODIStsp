#' @title test rsesults of tests against MD5 of test outputs
#' @description internal function which tests if the output of a test
#'  corresponds to the expected one.
#' @details When running in test mode, MODIStsp is expected to produce files
#'   with the same MD5 values as test files created using a stable (working)
#'   version
#' @param test integer number of the performed test
#' @rdname MODIStest_check_md5
#' @return NULL
#'
...
MODIStest_check_md5 <- function(test) {
  ...
  exp_md5 <- as.data.frame(
    RJSONIO::fromJSON(
      system.file("Test_files", "tests_md5.json", package = "MODIStsp")
    ),
  ...
  for (i in 1:nrow(exp_md5)) {
    ...
    if (length(grep("\\.vrt$", sel_file)) > 0) {
      ...
      writeLines(vrt_content[!c(1:length(vrt_content)) %in% lines_todelete],
                 sel_file)
    }

    sel_md5 <- exp_md5[i, "md5"]
    if (!file.exists(sel_file)) {
      warning(paste0("Error: file '",
                     sel_file,
                     "' was not created in the current temporary directory."))
      n_err <- n_err + 1
    } else if (sel_md5 != tools::md5sum(sel_file)) {
      warning(paste0("Error: file '",
                     sel_file,
  ...
  if (n_err == 0) {
    message(paste0("Test ", test, " completed succesfully.\n\n"))
  } else {
    message(paste0("Test ",
                   test,
                   " failed on ", n_err, "/", nrow(exp_md5), " files.\n\n"))
  }
}
