...
MODIStsp_GUI <- function(general_opts,
  ...
  prepare_to_save_options <- function(general_opts,
                                      gui_env,
                                      ...) {
    # workaround to retrieve custom index, since it was already save to the JSON but it is
    # not available in current variables

    general_opts$custom_indexes <- RJSONIO::fromJSON(previous_jsfile)$custom_indexes
    # retrieve product options
    general_opts$sel_prod     <- mod_prod_list[which(mod_prod_list == svalue(prod_wid))]
    general_opts$prod_version <- prod_opt_list[[general_opts$sel_prod]][[which(
      sapply(
        prod_opt_list[[general_opts$sel_prod]],
        function(x){
          x$v_number
        }
      ) == svalue(vers_wid))]]$v_number
    ...
    general_opts$bbox        <- c(svalue(output_ul_east_wid), svalue(output_lr_north_wid),
                                  svalue(output_lr_east_wid), svalue(output_ul_north_wid))
    ...
    if (general_opts$delete_hdf == "Yes") {
      gui_env$check_save_opts <- gconfirm("Warning! HDF files in Original MODIS folder will be
    ...
    if (general_opts$full_ext == "Resized" & gui_env$check_save_opts == TRUE) {
      bbox_mod         <- reproj_bbox(general_opts$bbox,
                                      ...
                                      enlarge = TRUE)
      d_bbox_mod_tiled <- crop(modis_grid, extent(bbox_mod))
      required_tiles  <- paste0("H",
                                apply(expand.grid(
                                  "H" = min(d_bbox_mod_tiled$H):max(d_bbox_mod_tiled$H),
                                  "V" = min(d_bbox_mod_tiled$V):max(d_bbox_mod_tiled$V)
                                ), 1, paste, collapse = "_V"))

      selected_tiles <- paste0("H",
                               apply(expand.grid(
      ...
      if (!any(required_tiles %in% selected_tiles)) {
        gui_env$check_save_opts <- gconfirm(
          strwrap("The selected tiles do not intersect the output bounding box.
                Do you want to discard your choice and automatically retrieve
                the required tiles?", width = 1000),
          handler = function(h, ...) {
      ...
      if (!all(required_tiles %in% selected_tiles) & gui_env$check_save_opts) {
        gconfirm(
          paste(strwrap("The following tiles not currently selected are required to
                        cover the output bounding box ("),
                paste(required_tiles[!(required_tiles %in% selected_tiles)], collapse = ", "),
                strwrap("). Do you want to add them to the processing? Otherwise, nodata will
                        be produced in the non-covered area.")),
          handler = function(h, ...) {
            selected_tiles       <<- required_tiles
            general_opts$start_x <<- min(d_bbox_mod_tiled$H)
            general_opts$end_x   <<- max(d_bbox_mod_tiled$H)
      ...
      if (!all(selected_tiles %in% required_tiles) & gui_env$check_save_opts) {
        gconfirm(
          paste("The following tiles are not required to cover the output bounding box (",
    ...
    if (general_opts$resampling == "mode" & gui_env$check_save_opts) {
      check_mode <- gconfirm(
        strwrap("Warning! You selected 'mode' resampling. Be aware that 'mode' resampling can
                provide inconsistent results in areas affected by mixed high and low
                quality data, and fail in properly keeping track of quality indicators!
                Do you wish to continue ?", width = 1000, simplify = TRUE), title = "Warning"
        )
      if (check_mode == FALSE) {
        gui_env$check_save_opts <- FALSE
      }
    ...
    if (general_opts$download_server == "http" &
      ...
      gmessage(
        message = strwrap("Username and password are mandatory in case of 'http' download !
                           Please provide them or choose 'ftp' download."),
        title   = "Warning")
      gui_env$check_save_opts <- FALSE
    }
  ...
  gui_env      <- new.env()   # create a new env to facilitate values-passing between widgets
  gui_env$quit <- TRUE
  ...
  main_win <- gbasicdialog(title = paste0("MODIStsp - v. ", packageVersion("MODIStsp")),
                           parent = NULL,
                           do.buttons = FALSE)

  # frame1 and 2 with expand=FALSE grant that widgets are not "too much expanded", neither
  ...
  mod_prod_cat <- as.data.frame(
    t(sapply(prod_opt_list, function(x){
  ...
  out_proj_list  <- hash(
    "Sinusoidal"   = strwrap("+proj=sinu +lon_0=0 +x_0=0 +y_0=0 +a=6371007.181 +b=6371007.181
                             +units=m +no_defs"),
    "UTM 32N"      = strwrap("+init=epsg:32632 +proj=utm +zone=32 +datum=WGS84 +units=m
                             +no_defs +ellps=WGS84 +towgs84=0,0,0"),
    "Latlon WGS84" = strwrap("+init=epsg:4326 +proj=longlat +datum=WGS84
                             +no_defs +ellps=WGS84 +towgs84=0,0,0"),
    "Latlon MODIS" = "+init=epsg:4008 +proj=longlat +ellps=clrk66 +no_defs",
    "User Defined" = ""
  )
  mod_proj_str <- strwrap("+proj=sinu +lon_0=0 +x_0=0 +y_0=0
  ...
  prod_wid <- gdroplist(
    ...
    selected  = match(sel_prod, mod_prod_list[mod_prod_cat$cat == svalue(cat_wid)]),
    handler   = function(h, ...) {
      ...
      # On product change, automatically modify the default projection - latlon for tiled, Sinu for
      # nontiled
      if (prod_opt_list[[sel_prod]][[svalue(vers_wid)]]$tiled == 0) {
        enabled(tiles_group) <- FALSE
        svalue(proj_wid)     <- "Latlon MODIS"
  ...
  if (prod_opt_list[[sel_prod]][[general_opts$prod_version]]$combined == 1) {
    enabled(sens_wid) <- FALSE
  } else {
    sens_wid[]       <- c("Terra", "Aqua", "Both")
    svalue(sens_wid) <- general_opts$sensor
  ...
  vers_wid   <- gcombobox(
    items     = sapply(prod_opt_list[[sel_prod]], function(x){
      x[["v_number"]]
    }),
  ...
  ##  ............................................................................
  ##  Upon click on the button we create a Child widget for selection of processing
  ##  layers (could try to separate a function for this, but it would be a hassle)
  ##  and deal with all events
  ...
  band_wid   <- gbutton(
    ...
    handler = function(h, ...) {
      ...
      check_names   <- prod_opt_list[[curr_prod]][[curr_vers]]$band_fullnames
      ...
      selgroup      <- gbasicdialog(
        ...
        #   - upon realizing the widget, the currently selected layers are ticked
        ...
        handler    = function(h, ...) {
          ...
          # Find which indexes selected and store in gui_env$temp_wid_bands_indexes
          ...
          # ? which quality selected ? --> store in gui_env$temp_wid_bands_quality array
      ...
      cbox       <- gframe(
        text       = "<span foreground='red' size='large'>Original MODIS Layers</span>",
        markup     = TRUE,
      ...
      check_names_quality <- prod_opt_list[[curr_prod]][[curr_vers]]$quality_fullnames
      if (!is.null(check_names_quality)) {
        check_wid_quality <- gui_env$temp_wid_bands_quality
        cbox_quality <- gframe(
          text       = "<span foreground='red' size='large'>Quality Indicators</span>",
          markup     = TRUE,
          container  = cbox_total,
          horizontal = FALSE
        )
      ...
      check_names_indexes <- c(
        prod_opt_list[[curr_prod]][[curr_vers]]$indexes_fullnames,
        as.list(general_opts$custom_indexes[[curr_prod]][[curr_vers]])$indexes_fullnames
      )
      if (!is.null(check_names_indexes)) {
        # retrieve currently selected indexes layers
        check_wid_indexes <- gui_env$temp_wid_bands_indexes
        cbox_indexes      <- gframe(
          text       = "<span foreground='red' size='large'>Additional Spectral Indexes</span>",
          markup     = TRUE,
          container  = cbox_total,
          horizontal = FALSE
        )
        ...
        ##  ............................................................................
        ##  Here we create the sub-child widget for creation of custom indexes      ####
        ##  The `MODIStsp_addindex` function is used to spawn a modal widget
        ##  for indexes creation
        ...
        band_wid_newindex  <- gbutton(
          ...
          handler = function(h, ...) {
            ...
            # since upon return the widget for layers selection is automatically disposed
            # to allow addition of the index, here we check and save which layers and
            # indexes are currently selected
            general_opts  <- RJSONIO::fromJSON(previous_jsfile)
            pos_wid       <- which(check_names %in% svalue(bands_wid))
            tmp_arr_bands <- array(data = 0, dim = length(check_names))
            tmp_arr_bands[pos_wid] <- 1
            gui_env$temp_wid_bands <- tmp_arr_bands
            # ? which indexes selected ? --> store in gui_env$temp_wid_bands_indexes array
            if (length(which(check_names_indexes != "") > 0)) {
              pos_wid     <- which(check_names_indexes %in% svalue(bands_wid_indexes))
              tmp_arr_ind <- array(data = 0, dim = length(check_names_indexes))
              tmp_arr_ind[pos_wid] <- 1
      ...
      www_but <- gbutton(
        ...
        handler   = function(button, ...) {
          browseURL(prod_opt_list[[curr_prod]][[curr_vers]]$www)
        }
      )
  ...
  aria_help <- gbutton(
    ...
    handler = function(h, ...) {
      ...
      help_mess_lab <- glabel(
        text = paste("If selected, use aria2c to accelerate download.",
                     "It is necessary that aria2c is installed and",
                     "that the binary is in the user PATH.",
                     "See https://aria2.github.io", sep = "\n"),
        editable  = FALSE,
        container = help_box
      )
  ...
  dates_frame <- gframe(
    text       = "<span foreground='red' size='x-large'>Processing Period</span>",
    markup     = TRUE,
    container  = main_group,
    horizontal = TRUE,
    expand     = TRUE
  ...
  seas_wid    <-  gcombobox(items     = seas_array,
                            container = dates_group,
  ...
  seas_help <- gbutton(
    ...
    handler = function(h, ...) {
      ...
      help_mess_lab <- glabel(
        text = paste(
          "If \"full\" is selected, all the available images between the starting",
  ...
  spatial_frame <- gframe(
    text       = "<span foreground='red' size='x-large'>Spatial Extent </span>",
    markup     = TRUE,
    container  = main_group,
    horizontal = FALSE,
    expand     = TRUE
  ...
  tiles_from_bbox <- gbutton(
    ...
    handler = function(h, ...) {
      bbox <- as.numeric(c(svalue(output_ul_east_wid), svalue(output_lr_north_wid),
                           svalue(output_lr_east_wid), svalue(output_ul_north_wid)))
      # Check if bbox is consistent
      ...
      if (svalue(output_ext_wid) != "Full Tiles Extent" & n_bbox_compiled == 0) {
        ...
        update_tiles(bbox,
                     ...
                     start_y_wid, end_y_wid) # If all checks pass, retrieve the tiles
      }
    },
    container = output_ext_group
  )
  ...
  bbox_from_file <- gbutton(
    ...
    handler = function(h, ...) {
      ...
      choice <- try(gfile(
        ...
        filter = list("Spatial files" = list(patterns = c("*.shp", "*.kml", "*.tif", "*.dat")),
      ...
      if (class(choice) != "try-error" & length(choice) != 0) {
          ...
          wait_window_lab <- glabel(text      = paste("Retrieving Extent, please wait..."),
                                    editable  = FALSE,
          ...
          # Create the bounding box in the chosen projection retrieving it from the
          # specified file
  ...
  output_proj_frame <- gframe(
    text       = "<span foreground='red' size='x-large'>Reprojection and Resize Options </span>",
    markup     = T,
    container  = main_group,
    horizontal = FALSE,
    expand     = TRUE
  ...
  proj_wid <- gcombobox(
    ...
    handler   = function(h, ...) {
      ...
      if (current_sel != "User Defined") {
        ...
        # If valid proj4string, and output is a bounding box, recompute the bounding box ----
        # in output proj coordinates, then update values in the text labels
        if (svalue(output_ext_wid) != "Full Tiles Extent") {
        ...
        # verify the inputted string. Revert to previous on error, otherwise modify projstring ----
        # and update the bounding box by converting coordinates to new out proj
        if (length(selproj) != 0 & selproj != "") {
          ...
          if (class(sel_output_proj) == "try-error") {
            gmessage(sel_output_proj,
                     title = "Proj4 String Not Recognized - Keeping the old output projection")
          } else {
  ...
  options_frame <- gframe(
    text       = "<span foreground='red' size='x-large'>Processing Options</span>",
    markup     = TRUE,
    container  = main_group,
    expand     = TRUE,
    horizontal = FALSE
  ...
  timeseries_wid <- gcombobox(
    ...
    container = opt_group,
    selected  = match(general_opts$ts_format, c("None", "ENVI Meta Files",
                                                "GDAL vrt Files", "ENVI and GDAL"))
  )
  ...
  nodata_wid <- gradio(items      = c("Yes", "No"),
                       text       = "Select",
                       container  = other_group,
                       selected   = match(general_opts$nodata_change, c("Yes", "No")),
                       horizontal = TRUE)
  ...
  scale_wid <- gradio(items      = c("Yes", "No"),
                      text       = "Select",
                      container  = other_group,
                      selected   = match(general_opts$scale_val, c("Yes", "No")),
                      horizontal = TRUE)
  ...
  scale_help <- gbutton(
    ...
    handler = function(h, ...) {
      ...
      help_mess_lab <- glabel(
        text = paste(
          "NASA provides outputs as integer values, indicating a potential scale factor",
  ...
  reprocess_wid       <- gradio(items      = c("Yes", "No"),
                                text       = "Select",
                                container  = outfold_group,
                                selected   = match(general_opts$reprocess, c("Yes", "No")),
                                horizontal = TRUE)
  ...
  load_but <- gbutton(
    ...
    handler   = function(h, ...){
      ...
      if (class(choice) != "try-error") {
        if (length(choice) == 0) {
          ...
          if (length(grep("\\.json$", choice)) == 0) {
            continue_load <- gconfirm("The selected file does not seem to be a JSON file.
      ...
      if (continue_load) {
        ...
        gmessage("Loading failed - Please select a different file !",
                 title = "Warning")
      }
    }
  )
    ...
    handler   = function(h, ...) {
      ...
      if (class(choice) != "try-error") {
        if (length(choice) == 0) {
          ...
          if (length(grep("\\.json$", choice)) == 0) {
            continue_save <- gconfirm("The selected file does not seem to be a JSON file.
                                      Do you want to continue?",
                                      title = "Warning",
                                      icon  = "warning")
          } else {
