...
MODIStsp <- function(gui               = TRUE,
  ...
  if (test > -1) {
    ...
    test_files <- sort(list.files(system.file("Test_files",
                                              package = "MODIStsp"),
                                  "^test[0-9]{2}\\.json$",
                                  full.names = TRUE))
    ...
    tests_hdf_zipped <- list.files(system.file("Test_files",
                                               package = "MODIStsp"),
                                   "\\.hdf\\.zip$",
                                   full.names = TRUE)
  ...
  # On interactive execution, check and load Rgtk2
  ...
  if (!gdal_hdf_support) {
    stop(paste(
      strwrap("Your local GDAL installation does not support HDF4 format.
      Please install HDF4 support and recompile GDAL. See: ", width = 800),
      strwrap("http://lbusett.github.io/MODIStsp/articles/installation.html#
      installing-r-and-gdal", width = 800))
    )
  }
  ...
  if (is.null(options_file) & gui == FALSE) {
    stop(strwrap("Please provide a valid \"options_file\" path value (or run
                 with gui=TRUE)."))
  }
  ...
  if (!exists("general_opts")) {
    general_opts <- list(
      ...
      # lenghts refearred to "Surf_Ref_8Days_500m (M*D09A1)" v6!
      bandsel         = rep(0, 13),
  ...
  if (!quit) {
    ...
    MODIStsp_process(
      ...
      quality_bit_n      = prod_opts$quality_bitN,
    ...
    # End If on "quit" --> If "quit", all of the above is skipped and program
    # terminates
